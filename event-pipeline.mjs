...
export class EventPipeline {
  ...
  persistAllPending(reason = 'shutdown') {
    let total = 0;
    for (const cat of this.priority) {
      while (this.queues[cat].length) {
        const ev = this.queues[cat].shift();
        this.writeFailed(cat, ev, `persistAllPending:${reason}`);
        this.metrics.failed[cat]++;
        total++;
      }
    }
    while (this.pendingMod.length) {
      const b = this.pendingMod.shift();
      this.writeFailed('moderation', b, `persistAllPending:${reason}:pendingMod`);
  ...
  enqueue(category, event) {
    ...
    if (this.queues[category].length >= max) {
      if (category === 'moderation') {
        // Bounded moderation queue: above max*1.5 we hard-drop to file and do NOT enqueue — memory stays bounded.
        if (this.queues[category].length >= max * 1.5) {
          this.writeFailed(category, event, 'queue_overflow_moderation_hard_limit');
          this.metrics.failed[category]++;
          this.log(`moderation overflow hard limit ${max*1.5} — persisted to file, not queued`);
          return;
        }
        // Below hard limit: persist overflow copy but still keep in queue (temporary expansion)
    ...
    if (category === 'moderation' && (event.type === 'ban' || event.type === 'mute')) this.suppress(event.targetId, Math.max(this.timing.suppressMs, AUDIT_WINDOW_MS));
