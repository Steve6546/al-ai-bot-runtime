...
const GOOD_CP = join(TMP, 'good.json');
{
  ...
  const rc = resolveRuntimeConfig({
    ...
    env: { DISCORD_TOKEN: 'test-token', GUILD_ID: '1523473815555018782' },
  });
  check('T-C5 guild from env', rc.guildId === '1523473815555018782');
  // legacy alias still works
  const rc2 = resolveRuntimeConfig({
    controlPlanePath: GOOD_CP,
    env: { DISCORD_TOKEN: 'test-token', OMNICORD_GUILD: '1523473815555018782' },
  });
  check('T-C5b legacy OMNICORD_GUILD alias', rc2.guildId === '1523473815555018782');
...
// ——— 4. env validation errors ———
{
  ...
  check('T-C9 missing env -> one error listing both keys', msg.includes('DISCORD_TOKEN') && msg.includes('GUILD_ID'), msg);

  let msg2 = '';
  try {
    resolveRuntimeConfig({ controlPlanePath: GOOD_CP, env: { DISCORD_TOKEN: 'x', GUILD_ID: 'not-a-snowflake' } });
  } catch (e) { msg2 = e.message; }
  check('T-C10 non-snowflake guild -> rejected', msg2.includes('GUILD_ID') && msg2.includes('17-20'), msg2);

  let msg3 = '';
  const saved = { ...process.env };
  delete process.env.DISCORD_TOKEN; delete process.env.GUILD_ID; delete process.env.OMNICORD_GUILD; delete process.env.ADAPTER_TOKEN;
  try { requireEnv(['DISCORD_TOKEN', 'GUILD_ID', 'ADAPTER_TOKEN']); }
