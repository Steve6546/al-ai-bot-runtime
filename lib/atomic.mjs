...
function atomicWrite(path, writeBody) {
...
/** Durable atomic JSON write: tmp + fsync + rename (no pre-unlink gap). */
export function atomicWriteJson(path, data) {
  atomicWrite(path, fd => writeFileSync(fd, JSON.stringify(data, null, 2)));
}

/** Durable atomic raw-text write — same guarantees as atomicWriteJson. */
export function atomicWriteText(path, text) {
  atomicWrite(path, fd => writeFileSync(fd, text));
}
